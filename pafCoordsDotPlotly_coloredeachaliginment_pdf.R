...
rseqlengths = tapply(alignments$refLen, alignments$refID, max) # get length of each refID
qseqlengths = tapply(alignments$queryLen, alignments$queryID, max) # get length of each queryID
...
desired <- as.character(levels(alignments$queryID))
queryMax <- qseqlengths[ desired ]

alignments$queryStart2 = alignments$queryStart + sapply(as.character(alignments$queryID), function(x) ifelse(x == names(queryMax)[1], 0, cumsum(as.numeric(queryMax))[match(x, names(queryMax)) - 1]) )
alignments$queryEnd2 = alignments$queryEnd +     sapply(as.character(alignments$queryID), function(x) ifelse(x == names(queryMax)[1], 0, cumsum(as.numeric(queryMax))[match(x, names(queryMax)) - 1]) )

# plot
yTickMarks <- sapply(1:length(queryMax), function(x) {
    return(cumsum(as.numeric(queryMax))[x])
  }
)
...
if (opt$similarity) {
  gp = ggplot(alignments) +
    ...
    scale_x_continuous(expand = c(0, 0),breaks = c(0,cumsum(as.numeric(chromMax[names(chromMax) %in% levels(alignments$refID)]))),
                       labels = c("0",levels(alignments$refID))) +
    theme_bw() +
    ...
    scale_y_continuous(expand = c(0, 0),breaks = c(0,yTickMarks), 
    labels = c("0",substr(levels(alignments$queryID), start = 1, stop = 20))) +
    { if(opt$h_lines){ geom_hline(yintercept = c(0,yTickMarks),
                                  color = "grey60",
                                  linewidth = .1) }} +
    scale_color_gradient2(low = "brown", mid = "orange", high = "green", limits = c(0, 1), midpoint = 0.5) +
